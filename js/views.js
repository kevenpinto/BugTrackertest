...
class CreateUser {
  ...
  view() {
    return m(UserEditor, {
      email: '',
      passwd: '',
      onSubmit: async ({email,passwd}) => {
        await this.model.createUser({email: email, passwd: passwd})
        m.route.set(`/issues`)
        m.redraw()
      }
    })
...
class UserEditor {
  constructor(vnode) {
    ...
    this.passwd = vnode.attrs.passwd
    this.onSubmit = vnode.attrs.onSubmit
  }
  view() {
    return m('form', {onsubmit: e => this.onSubmit({email: this.email, passwd: this.passwd})}, [
      m('.form-group', [
        m('label', {'for': 'email-input'}, 'Enter Email:'),
        m('input.input[type=email]', {oninput: (e) => {this.email = e.target.value}}),
        m('br'),
        m('small',{class:'form-text  text-muted'},"We'll never share your email with anyone else")
      ]),
      m('.form-group', [
        m('label', {'for': 'passwd-input'}, 'Password :'),
        m('input.input[type=password]', {oninput: (e) => {this.passwd = e.target.value}})
      ]),
...
//src="https://airelogic.com/wp-content/uploads/2017/07/aire-logic-300x94.png
  view() {
...
module.exports = {IssuesList, ViewIssue, EditIssue, CreateIssue, IssueEditor, ToolbarContainer, CreateUser,UserEditor,AboutPage,DashboardList}
